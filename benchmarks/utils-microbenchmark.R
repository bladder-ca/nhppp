...
nhppp_t_intensity_loop <- function() {
  t_ <- vector(mode = "numeric")
  while (length(t_) == 0) {
    t_ <- nhppp_t_intensity(lambda = l, lambda_maj = c(0, l(rngt[2])), range_t = rngt, only1 = TRUE)
  }
  return(t_)
}

zt_res_low <- bench::mark(
  "ztppp_t" = ztppp_t(rate = 0.1, range_t = rngt, only1 = TRUE), 
  "ppp_t loop" = ppp_t_loop(),

  "ztnhppp_t_cumulative_intensity" = ztnhppp_t_cumulative_intensity(Lambda = L, Lambda_inv = Li, range_t = rngt, range_L = L(rngt), only1= TRUE),
  "nhppp_t_cumulative_intensity_inversion loop" = nhppp_t_cumul_intensity_inversion_loop(),
  "nhppp_t_cumulative_intensity_orderstats loop" = nhppp_t_cumul_intensity_orderstats_loop(),
  
  "ztnhppp_t_intensity" = ztnhppp_t_intensity(lambda = l, lambda_maj = c(0, l(rngt[2])), range_t = rngt),
  "nhppp_t_intensity loop" = nhppp_t_intensity_loop(),
