...
#'
#' @examples
#' x <- draw_zt_intensity_step(lambda = function(t) exp(.02 * t))
draw_zt_intensity_step <- function(lambda,
  ...
  while (TRUE) {
    candidate_times <- ppp_t_step(rates_vector = lambda_maj_vector, times_vector = times_vector, rng_stream = rng_stream, only1 = FALSE, zero_truncated = TRUE)
