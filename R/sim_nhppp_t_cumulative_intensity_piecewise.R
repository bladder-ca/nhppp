...
#'
#' @param Lambda_matrix (scalar, double) `K` integrated rates, one per interval
...
#'
#' @examples
#' x <- nhppp_t_piecewise_regular_vec(Lambda_vector = 1:5)
#' @export
nhppp_t_piecewise_regular_vec <- function(Lambda_matrix,
  ...
  Tau <- matrix(data = stats::rexp(n = n_draws * n_max_events, rate = 1),
  ...
  # possibly overwrite Tau in place -- WE DO NOT NEED Z
  ...
  for (ev in 1:n_max_events){
    ...
    Tau[,ev] <- range_t[1] +
      ((IntervalIndex[, ev+1] - 1) +  # 0-based indexing of intervals here
         (Tau[,ev] - Lambda_matrix[IntervalIndex[,c(1, ev+1)]]) /
         (Lambda_matrix[IntervalIndex[,c(1, ev+1)] + AddOneForNextIndex] -
          Lambda_matrix[IntervalIndex[,c(1, ev+1)]])
      ) * interval_duration
  }
  return(Tau)
}
