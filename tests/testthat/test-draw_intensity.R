test_that("draw_intensity() works", {
  ...
  expect_no_error(withr::with_preserve_seed(df <- draw_intensity(lambda = l, majorizer_intercept = 2.2, majorizer_slope = 0, t_min = 0, t_max = 10)))
  check_ppp_sample_validity(times = df, t_min = 0, t_max = 10)

  # works when range_t[1]>0
  expect_no_error(withr::with_preserve_seed(df <- draw_intensity(lambda = l, majorizer_intercept = 2.2, majorizer_slope = 0, t_min = 5, t_max = 10)))
