test_that("ppp_orderstat() works", {
  expect_no_error(
    withr::with_seed(
      12345,
      df1 <- ppp_orderstat(range_t = c(0, 10), rate = 1)
    )
  )
  ...
  expect_no_error(df2 <- ppp_orderstat(range_t = c(0, 10), rate = 1, rng_stream = S1))
  check_ppp_sample_validity(times = df2, t_min = 0, t_max = 10)

  # # check RNGClass
  # expect_no_error(S2 <- readRDS(file.path("example_RNGCLass.rds"))$unpack())
  # expect_no_error(df3 <- ppp_orderstat(range_t = c(0, 10), rate = 1, rng_stream = S2))
