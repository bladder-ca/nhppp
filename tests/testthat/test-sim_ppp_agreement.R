...
test_that("ppp_t_piecewise() agrees with strung together constant rates", {
  r_ppp_t <- unlist(lapply(integer(10000),
                           function(x){
                             c(ppp_t(range_t = c(1,   2), rate =  1, only1 = FALSE),
  ...
  r_ppp_t1 <- unlist(lapply(integer(10000),
                           function(x){
                             c(ppp_t(range_t = c(1,   2), rate =  1, only1 = TRUE),
                               ppp_t(range_t = c(2, 2.2), rate = 10, only1 = TRUE),
                               ppp_t(range_t = c(2.2, 3), rate =  3, only1 = TRUE))[1]
                           } ))
