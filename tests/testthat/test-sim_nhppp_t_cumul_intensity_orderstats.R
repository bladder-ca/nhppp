test_that("nhppp_t_cumulative_intensity_orderstats() works", {
  L <- function(t) {
    return(2 * t)
  }
  Li <- function(z) {
  ...
  expect_no_error(withr::with_preserve_seed(df <- nhppp_t_cumulative_intensity_orderstats(Lambda = L)))
  check_ppp_sample_validity(times = df, t_min = 0, t_max = 10)

  # works when Lambda_inv() is given
  expect_no_error(withr::with_preserve_seed(df1 <- nhppp_t_cumulative_intensity_orderstats(Lambda = L, Lambda_inv = Li)))
  check_ppp_sample_validity(times = df1, t_min = 0, t_max = 10)

  # works when range_t[1]>0
  expect_no_error(withr::with_preserve_seed(df2 <- nhppp_t_cumulative_intensity_orderstats(Lambda = L, Lambda_inv = Li, range_t = c(5, 10))))
  ...
  check_ppp_sample_validity(times = df4, t_min = 5, t_max = 10, only1 = TRUE)
})
