...
test_that("ztnhppp_intensity_piecewise() works", {
  ...
  expect_no_error(withr::with_preserve_seed(df <- draw_zt_intensity_step(lambda = l, times_vector = times, lambda_maj_vector = lambda_maj)))
  check_ppp_sample_validity(times = df, t_min = 0, t_max = 10, zero_truncated = TRUE)

  # works when range_t[1]>0
  times2 <- c(0, 1, pi, 2 * pi, 10) / 2 + 5
  expect_no_error(withr::with_preserve_seed(df <- draw_zt_intensity_step(lambda = l, times_vector = times2, lambda_maj_vector = lambda_maj)))
  check_ppp_sample_validity(times = df, t_min = 5, t_max = 10, zero_truncated = TRUE)

  # works with only1 = TRUE
  expect_no_error(
    withr::with_preserve_seed(
      df1 <- draw_zt_intensity_step(
  ...
  # works with rstream generator
  expect_no_error(withr::with_preserve_seed(df2 <- draw_zt_intensity_step(lambda = l, times_vector = times2, lambda_maj_vector = lambda_maj, rng_stream = S)))
  check_ppp_sample_validity(times = df2, t_min = 5, t_max = 10, zero_truncated = TRUE)
})
