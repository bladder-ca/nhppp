test_that("vdraw_intensity_step() samples on irregular grids", {
  ...
  Z <- vdraw_intensity_step(
    lambda = lfun,
    lambda_maj_matrix = 1.5 * lmaj[1:10, ],
...
test_that("conditioned thinning agrees across grids and majorizers", {
  ...
  Z_reg <- vdraw_intensity(
    lambda = lfun,
    lambda_maj_matrix = matrix(rep(1.2, 5 * n_draws), ncol = 5),
    rate_matrix_t_min = 1,
    rate_matrix_t_max = 5,
  ...
  Z_loose <- vdraw_intensity_step(
    lambda = lfun,
    lambda_maj_matrix = matrix(rep(3, 5 * n_draws), ncol = 5),
    time_breaks = breaks,
    generate_at_least_K = 2
  )
  compare_ppp_vectors(
  ...
  Z <- vdraw_intensity_step(
    lambda = lfun,
    lambda_maj_matrix = matrix(rep(1.2, 5 * 200), ncol = 5),
    time_breaks = breaks,
    generate_at_least_K = 2,
    report_first_K = 2
  )
