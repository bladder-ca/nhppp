test_that("draw_intensity_step() works", {
  ...
  expect_no_error(withr::with_preserve_seed(df <- draw_intensity_step(lambda = l, time_breaks = times, lambda_maj_vector = lambda_maj)))
  check_ppp_sample_validity(times = df, t_min = 0, t_max = 10)

  # works when range_t[1]>0
  times2 <- c(0, 1, pi, 2 * pi, 10) / 2 + 5
  expect_no_error(withr::with_preserve_seed(df <- draw_intensity_step(lambda = l, time_breaks = times2, lambda_maj_vector = lambda_maj)))
  ...
  expect_no_error(
    withr::with_preserve_seed(
      df1 <- draw_intensity_step(
        lambda = l, time_breaks = times2, lambda_maj_vector = lambda_maj, atmost1 = TRUE
      )
    )
  )
