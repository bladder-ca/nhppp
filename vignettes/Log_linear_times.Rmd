...
```{r function-lambda-0} 
# first positional argument is `t`
# second positional argument is a named list: 
#   list(shared = ..., row_args = ...)
l <- function(t, args) {
 alpha <- args$row_args$alpha
 beta <- args$row_args$beta
 exp(alpha + beta * t)
}
```


For example, here we use vectorized arguments, using the first 5 people and evaluating all people at `t = 45`
```{r}
l(t = 45, list(row_args = pop[1:5, .(alpha, beta)]))
```
...
```{r}
l(t = matrix(c(45, 50, 45, 47.4, 30), ncol = 1), 
  args = list(row_args = pop[1:5, .(alpha, beta)])
)
``` 
...
t_mat <- matrix(c(
  ...
  50.1, 45.5, 47.8, 
    38,   48, 52.7, 
  60.1, 70.1, 99.9), ncol = 3, byrow = TRUE)
t_mat

l(t = t_mat, args = list(row_args = pop[1:5, .(alpha, beta)]))
```


Analogusly, we can evaluate the 
intensity function $\lambda()$ for the whole population passing the `t`
argument, and the whole population's `alpha` and `beta` variables.

```{r}
t_mat <- matrix(rep(c(45, 50, 55), each = K), ncol = 3, byrow = TRUE)
l(t = t_mat, 
 args = list(row_args = pop[, .(alpha, beta)])) |> head()
```
...
(The `budget_cap` option can be useful to speed up the sampling and minimize memory 
...
In this example, `budget_cap = 5` is fine -- it returns exact solutions; but we 
...
lmaj_mat <- nhppp::get_step_majorizer(
  ...
  breaks = breaks_mat,
  is_monotone = TRUE, 
  fun_args = list(row_args = pop[,.(alpha, beta)])
)
...
pop[
  ...
  t_thinning := nhppp::vdraw_intensity(
    lambda = l,
    lambda_args = list(row_args = pop[,.(alpha, beta)]),
...
3. `r tictoc::tic.log()[[3]]`. This approach is typically times faster that the second one in most problems,  
