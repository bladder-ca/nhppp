...
```{r}
t_mat <- matrix(c(45, 50, 45, 47.4, 30, 
               45.1, 50.1, 45.5, 47.8, 38, 
               48, 52.7, 60.1, 70.1, 99.9), ncol = 3, byrow = TRUE); t_mat
...
For the log-linear intensity in this example, some calculus yields 
...
It's arguments `alpha`, `beta` have the same name as the parameters 
in our log-linear intensity function $\lambda$. The `range_t` argument asks for
...
The `nhppp::vdraw_intensity_step_regular_cpp` function assumes that 
...
a majorizer matrix over $M=5$ intervals over the whole simulation horizon. You 
specify the $[T_{k0}, T_{k1})$ intervals for each person via the `subinterval` option. 
...
pop[,
    t_thinning := nhppp::vdraw_intensity_step_regular_cpp(
      lambda = l,
      lambda_maj_matrix = lmaj_mat,
      range_t = c(40, 100),
      subinterval =  matrix(c(pop$T0, pop$T1), ncol = 2, byrow = FALSE),
      atmost1 = TRUE, 
      atmostB = 5
      )
]
...
range_t <- matrix(c(pop$T0, pop$T1), ncol = 2, byrow = FALSE)
pop[,
...
The QQ plots comparing the simulated times with the three methods -- and 
there is good agreement over this population of size $K = `r K`$. As $K$ increases 
the agreement remains evidently excellent. We have done in-depth comparisons elsewhere. 
...
Thanks to Carolyn Rutter and Hui Hsuan Chan for providing the numerical example we use in this vignette. 
