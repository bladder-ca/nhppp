...
ppp_t(
  range_t = c(0, 10), rate = 1, tol = 10^-6, only1 = FALSE,
  rng_stream = S
)
@
...
<<demo-draw_intensity_piecewise>>=
draw_intensity_piecewise(
  lambda = lambda_fun,
...
<<demo-draw_zt_cumulative_intensity>>=
...
<<demo-ppp_t_piecewise>>=
ppp_t_piecewise(
  rates_vector = 1:5, times_vector = 0:5, only1 = FALSE,
...
<<demo-zt-draw_intensity_linear>>=
draw_intensity_linear(
  alpha = 3, beta = -0.5, range_t = c(0, 10),
  only1 = FALSE
)
draw_zt_intensity_linear(
...
<<demo-draw_intensity_loglinear>>=
draw_intensity_loglinear(
  alpha = 1, beta = -0.02, range_t = c(8, 10),
  only1 = FALSE
)
@

<<demo-draw_zt_intensity_loglinear>>=
draw_zt_intensity_loglinear(
  alpha = 1, beta = -0.02, range_t = c(5, 10),
  only1 = FALSE
)
@
...
l_m <- get_piecewise_linear_majorizer(
...
simulate_nhppp_pkg <- function() {
  ...
  Thinning_star <- nhppp::draw_intensity_piecewise(
    lambda = l,
    lambda_maj_vector = l_star(t_breaks[1:M]),
    times_vector = t_breaks,
    only1 = FALSE
  )
  Thinning_star2 <- nhppp::draw_intensity_piecewise(
    lambda = l,
...
comptimes_all_samples <- bench::mark(
  "(1a) nhppp: thinning (majorizer a)" = nhppp::draw_intensity(
    lambda = l, lambda_maj = l(6 * pi), range_t = c(0, 6 * pi), only1 = FALSE
  ),
  "(1b) nhppp: thinning (majorizer b)" = nhppp::draw_intensity_piecewise(
    lambda = l, lambda_maj_vector = l_star(t_breaks[1:M]),
    times_vector = t_breaks, only1 = FALSE
  ),
  "(1c) nhppp: thinning (majorizer c)" = nhppp::draw_intensity_piecewise(
    lambda = l, lambda_maj_vector = l_star2(t_breaks[1:M]),
    times_vector = t_breaks, only1 = FALSE
  ),
  "(1d) nhppp: inversion (with inverse)" =
    nhppp::draw_cumulative_intensity_inversion(
      Lambda = L, Lambda_inv = Li, range_t = c(0, 6 * pi), only1 = FALSE
    ),
  "(1e) nhppp: inversion (no inverse)" =
    nhppp::draw_cumulative_intensity_inversion(
      Lambda = L, Lambda_inv = NULL, range_t = c(0, 6 * pi), only1 = FALSE
    ),
...
comptimes_one_sample <- bench::mark(
  "(1a) nhppp: thinning (majorizer a)" = nhppp::draw_intensity(
    lambda = l, lambda_maj = l(6 * pi), range_t = c(0, 6 * pi), only1 = TRUE
  ),
  "(1b) nhppp: thinning (majorizer b)" = nhppp::draw_intensity_piecewise(
    lambda = l, lambda_maj_vector = l_star(t_breaks[1:M]),
    times_vector = t_breaks, only1 = TRUE
  ),
  "(1c) nhppp: thinning (majorizer c)" = nhppp::draw_intensity_piecewise(
    lambda = l, lambda_maj_vector = l_star2(t_breaks[1:M]),
    times_vector = t_breaks, only1 = TRUE
  ...
  "(1d) nhppp: inversion (with inverse)" =
    nhppp::draw_cumulative_intensity_inversion(
      Lambda = L, Lambda_inv = Li, range_t = c(0, 6 * pi), only1 = TRUE
    ),
  "(1e) nhppp: inversion (no inverse)" =
    nhppp::draw_cumulative_intensity_inversion(
      Lambda = L, Lambda_inv = NULL, range_t = c(0, 6 * pi), only1 = TRUE
    ),
  "(1f) nhppp: order statistics (with inverse)" =
    nhppp::draw_cumulative_intensity_orderstats(
      Lambda = L, Lambda_inv = Li, range_t = c(0, 6 * pi), only1 = TRUE
...
<<app-majorizer>>=
get_piecewise_linear_majorizer(
