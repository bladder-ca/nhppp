...
\hlkwd{draw_sc_step_regular}\hlstd{(}
  \hlkwc{lambda_vector} \hlstd{=} \hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{range_t} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0.5}\hlstd{,} \hlnum{5.9}\hlstd{),} \hlkwc{only1} \hlstd{=} \hlnum{FALSE}\hlstd{,}
...
Functions \fct{draw\_sc\_linear} and \fct{ztdraw\_sc\_linear} sample zero or more and at least one event, respectively, from NHPPPs with linear intensity functions. An optional argument (\texttt{only1}) returns the first event only.
\begin{align*}
    \lambda(t) =
    \begin{cases}
...
\begin{tabular}{rlllll}
 ...
 & Thinning $\lambda_*$=$\lambda_{*a}$ & Thinning $\lambda_*$=$\lambda_{*b}$ & Thinning $\lambda_*$=$\lambda_{*c}$ & Inversion & Order statistics \\ 
  \hline
Sample mean & 172.440 & 170.030 & 172.020 & 170.350 & 173.680 \\ 
  $B_\mu$ & 1.305 & -1.105 & 0.885 & -0.785 & 2.545 \\ 
  $B_{\mu, rel}$ & 0.763 & -0.646 & 0.517 & -0.459 & 1.487 \\ 
  Sample variance & 171.865 & 196.938 & 187.333 & 165.745 & 202.947 \\ 
  $B_V$ & 0.730 & 25.804 & 16.198 & -5.390 & 31.812 \\ 
  $B_{V, rel}$ & 0.427 & 15.078 & 9.465 & -3.149 & 18.589 \\ 
  Goodness of fit, $\chi^2$ [$p$~value] & 4.090 [1.000] & 4.263 [1.000] & 3.563 [1.000] & 4.269 [1.000] & 5.572 [1.000] \\ 
  $W_1$ [$p$~value] & 1.759 [0.204] & 1.427 [0.356] & 1.325 [0.074] & 1.244 [0.066] & 2.631 [0.114] \\ 
  Equal tail 95\% CI = [146, 197] & [148, 192] & [143, 194] & [145, 201] & [146, 197] & [149, 200] \\ 
  Equal tail 90\% CI = [150, 193] & [151, 192] & [147, 193] & [150, 193] & [150, 192] & [152, 195] \\ 
  Equal tail 75\% CI = [156, 186] & [159, 187] & [153, 186] & [157, 185] & [154, 183] & [157, 188] \\ 
  Equal tail 50\% CI = [162, 180] & [163, 183] & [161, 180] & [162, 180] & [162, 177] & [163, 183] \\ 
   \hline
\end{tabular}
}
...
\begin{tabular}{rll}
 ...
 & Goodness of fit, $\chi^2$ [$p$~value] & $W_1$ [$p$~value] \\ 
  \hline
Thinning $\lambda_*$=$\lambda_{*a}$ & 0.008 [1.000] & 0.561 [1.000] \\ 
  Thinning $\lambda_*$=$\lambda_{*b}$ & 0.009 [1.000] & 0.378 [0.970] \\ 
  Thinning $\lambda_*$=$\lambda_{*c}$ & 0.011 [1.000] & 0.452 [1.000] \\ 
  Inversion & 0.009 [1.000] & 0.374 [0.996] \\ 
  Order statistics & 0.009 [1.000] & 0.407 [1.000] \\ 
   \hline
\end{tabular}
}
...
$51$ times faster than the non-vectorized function ($1.15 \mu s$ versus $59.01 \mu s$ per sample drawn.)
%
However, when drawing all events, the order changes: the vectorized function is approximately $2.5$ times slower than the non-vectorized function ($1.23 ms$ versus $0.50 ms$ per set of event times drawn.) The reversal in apparent performance is because the vectorized function is not optimized for large numbers of events per draw. (The expected number of events with $\lambda_{*b}$ in $(0, 6\pi]$ is
$741.97$.)
