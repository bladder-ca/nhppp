...
%
When drawing all events, the vectorized function is approximately $\Sexpr{pprint(as.numeric(comptimes_vec[[1,"median"]]) / as.numeric(comptimes_vec[[2,"median"]]), 1) }$ times faster than the non-vectorized function (median $\Sexpr{pprint(as.numeric(comptimes_vec[[2,"median"]]) *10^3)} ms$ versus $\Sexpr{pprint(as.numeric(comptimes_vec[[1,"median"]]) *10^3)} ms$ over $\Sexpr{n_simulations}$ simulations). (The expected number of events with $\lambda_{*b}$ in $(0, 6\pi]$ is
