...
<<demo-nhppp_t_intensity_piecewise>>=
nhppp_t_intensity_piecewise(
  lambda = lambda_fun,
...
<<demo-nhppp_t_cumulative_intensity_inversion>>=
...
<<demo-nhppp_t_cumulative_intensity_orderstats>>=
nhppp_t_cumulative_intensity_orderstats(
  Lambda = Lambda_fun,
  Lambda_inv = Lambda_inv_fun,
  range_t = c(4.1, 7.6),
  only1 = FALSE
...
<<demo-ztnhppp_t_cumulative_intensity>>=
...
<<demo-zt-nhppp_t_intensity_linear>>=
nhppp_t_intensity_linear(
  alpha = 3, beta = -0.5, range_t = c(0, 10),
  only1 = FALSE
)
ztnhppp_t_intensity_linear(
...
<<demo-nhppp_t_intensity_exponential>>=
nhppp_t_intensity_exponential(
  alpha = 1, beta = -0.02, range_t = c(8, 10),
  only1 = FALSE
)
@

<<demo-ztnhppp_t_intensity_exponential>>=
ztnhppp_t_intensity_exponential(
...
Li <- approxfun(x = L(seq(0, 6 * pi, 10^-3)),
...
simulate_nhppp_pkg <- function() {
  Thinning_const <- nhppp::nhppp_t_intensity(
    lambda = l,
    lambda_maj = l(6 * pi),
    range_t = c(0, 6 * pi),
    only1 = FALSE
  )
  Thinning_star <- nhppp::nhppp_t_intensity_piecewise(
    lambda = l,
    lambda_maj_vector = l_star(t_breaks[1:M]),
    times_vector = t_breaks,
    only1 = FALSE
  )
  Thinning_star2 <- nhppp::nhppp_t_intensity_piecewise(
    lambda = l,
...
comptimes_all_samples <- bench::mark(
  "(1a) nhppp: thinning (majorizer a)" = nhppp::nhppp_t_intensity(
    lambda = l, lambda_maj = l(6 * pi), range_t = c(0, 6 * pi), only1 = FALSE
  ),
  "(1b) nhppp: thinning (majorizer b)" = nhppp::nhppp_t_intensity_piecewise(
    lambda = l, lambda_maj_vector = l_star(t_breaks[1:M]),
    times_vector = t_breaks, only1 = FALSE
  ),
  "(1c) nhppp: thinning (majorizer c)" = nhppp::nhppp_t_intensity_piecewise(
  ...
  "(1f) nhppp: order statistics (with inverse)" =
    nhppp::nhppp_t_cumulative_intensity_orderstats(
      Lambda = L, Lambda_inv = Li, range_t = c(0, 6 * pi), only1 = FALSE
    ),
