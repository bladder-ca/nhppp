...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{nhppp_t_intensity_piecewise}\hlstd{(}\hlkwc{lambda} \hlstd{= lambda_fun,}
...
\hlkwd{nhppp_t_cumulative_intensity_inversion}\hlstd{(}\hlkwc{Lambda} \hlstd{= Lambda_fun,}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{nhppp_t_cumulative_intensity_orderstats}\hlstd{(}\hlkwc{Lambda} \hlstd{= Lambda_fun,}
  \hlkwc{Lambda_inv} \hlstd{= Lambda_inv_fun,}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{nhppp_t_intensity_linear}\hlstd{(}\hlkwc{alpha} \hlstd{=} \hlnum{3}\hlstd{,} \hlkwc{beta} \hlstd{=} \hlopt{-}\hlnum{0.5}\hlstd{,} \hlkwc{range_t} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{10}\hlstd{),}
  \hlkwc{only1} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlkwd{ztnhppp_t_intensity_linear}\hlstd{(}\hlkwc{alpha} \hlstd{=} \hlnum{0.5}\hlstd{,} \hlkwc{beta} \hlstd{=} \hlnum{0.2}\hlstd{,} \hlkwc{range_t} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{10}\hlstd{),}
  \hlkwc{only1} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{nhppp_t_intensity_exponential}\hlstd{(}\hlkwc{alpha} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{beta} \hlstd{=} \hlopt{-}\hlnum{0.02}\hlstd{,} \hlkwc{range_t} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{8}\hlstd{,} \hlnum{10}\hlstd{),}
  \hlkwc{only1} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{ztnhppp_t_intensity_exponential}\hlstd{(}\hlkwc{alpha} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{beta} \hlstd{=} \hlopt{-}\hlnum{0.02}\hlstd{,} \hlkwc{range_t} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{5}\hlstd{,} \hlnum{10}\hlstd{),}
  \hlkwc{only1} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\begin{verbatim}
...
\begin{tabular}{rlllll}
 ...
 & Thinning $\lambda_*$=$\lambda_{*a}$ & Thinning $\lambda_*$=$\lambda_{*b}$ & Thinning $\lambda_*$=$\lambda_{*c}$ & Inversion & Order statistics \\ 
  \hline
Sample mean & 169.610 & 170.710 & 170.450 & 172.600 & 172.510 \\ 
  $B_\mu$ & -1.525 & -0.425 & -0.685 & 1.465 & 1.375 \\ 
  $B_{\mu, rel}$ & -0.891 & -0.248 & -0.400 & 0.856 & 0.804 \\ 
  Sample variance & 168.341 & 147.157 & 205.785 & 168.323 & 170.919 \\ 
  $B_V$ & -2.793 & -23.977 & 34.651 & -2.811 & -0.216 \\ 
  $B_{V, rel}$ & -1.632 & -14.011 & 20.248 & -1.643 & -0.126 \\ 
  Goodness of fit, $\chi^2$ [$p$~value] & 6.033 [1.000] & 5.164 [1.000] & 3.308 [1.000] & 6.281 [1.000] & 4.103 [1.000] \\ 
  $W_1$ [$p$~value] & 1.718 [0.012] & 1.209 [0.060] & 1.419 [0.102] & 1.839 [0.176] & 1.690 [0.506] \\ 
  Equal tail 95\% CI = [146, 197] & [141, 193] & [148, 194] & [144, 198] & [145, 194] & [150, 198] \\ 
  Equal tail 90\% CI = [150, 193] & [145, 191] & [150, 188] & [146, 192] & [149, 193] & [152, 195] \\ 
  Equal tail 75\% CI = [156, 186] & [156, 184] & [159, 184] & [155, 185] & [157, 189] & [157, 186] \\ 
  Equal tail 50\% CI = [162, 180] & [163, 176] & [162, 179] & [160, 181] & [164, 182] & [165, 181] \\ 
   \hline
\end{tabular}
}
...
\begin{tabular}{rp{1in}p{1in}p{1in}p{1in}p{1.1in}}
 ...
 & \pkg{reda} thinning, $\lambda_*$=$\lambda_{*a}$ & \pkg{reda} inversion & \pkg{simEd} thinning, $\lambda_*$=$\lambda_{*a}$ & \pkg{IndTestPP} thinning, no $\lambda_*$ & \pkg{IndTestPP} inversion \\ 
  \hline
Sample mean & 173.120 & 171.650 & 176.650 & 192.770 & 190.400 \\ 
  $B_\mu$ & 1.985 & 0.515 & 5.515 & 21.635 & 19.265 \\ 
  $B_{\mu, rel}$ & 1.160 & 0.301 & 3.223 & 12.642 & 11.257 \\ 
  Sample variance & 177.884 & 180.210 & 303.341 & 161.411 & 178.364 \\ 
  $B_V$ & 6.750 & 9.075 & 132.206 & -9.723 & 7.229 \\ 
  $B_{V, rel}$ & 3.944 & 5.303 & 77.253 & -5.682 & 4.224 \\ 
  Goodness of fit, $\chi^2$ [$p$~value] & 6.974 [1.000] & 3.407 [1.000] & 37.847 [1.000] & 282.531 [<0.001] & 224.760 [<0.001] \\ 
  $W_1$ [$p$~value] & 2.201 [0.086] & 0.958 [0.276] & 5.864 [<0.001] & 21.636 [0.152] & 19.266 [0.106] \\ 
  Equal tail 95\% CI = [146, 197] & [146, 198] & [147, 199] & [148, 215] & [171, 220] & [165, 216] \\ 
  Equal tail 90\% CI = [150, 193] & [148, 193] & [150, 194] & [148, 215] & [175, 213] & [166, 213] \\ 
  Equal tail 75\% CI = [156, 186] & [159, 188] & [155, 186] & [161, 198] & [179, 207] & [173, 205] \\ 
  Equal tail 50\% CI = [162, 180] & [165, 181] & [163, 180] & [167, 183] & [183, 202] & [180, 199] \\ 
   \hline
\end{tabular}
}
...
\begin{tabular}{rll}
 ...
 & Goodness of fit, $\chi^2$ [$p$~value] & $W_1$ [$p$~value] \\ 
  \hline
Thinning $\lambda_*$=$\lambda_{*a}$ & 0.008 [1.000] & 0.528 [0.992] \\ 
  Thinning $\lambda_*$=$\lambda_{*b}$ & 0.011 [1.000] & 0.603 [1.000] \\ 
  Thinning $\lambda_*$=$\lambda_{*c}$ & 0.008 [1.000] & 0.320 [0.996] \\ 
  Inversion & 0.007 [1.000] & 0.388 [0.992] \\ 
  Order statistics & 0.006 [1.000] & 0.226 [0.970] \\ 
   \hline
\end{tabular}
}
...
\begin{tabular}{rll}
 ...
 & Goodness of fit, $\chi^2$ [$p$~value] & $W_1$ [$p$~value] \\ 
  \hline
\pkg{reda} thinning ($\lambda_*$=$\lambda_{*a}$) & 0.008 [1.000] & 0.416 [1.000] \\ 
  \pkg{reda} inversion & 0.009 [1.000] & 0.283 [0.972] \\ 
  \pkg{simEd} thinning ($\lambda_*$=$\lambda_{*a}$) & 0.052 [1.000] & 0.227 [0.922] \\ 
  \pkg{IndTestPP} thinning (no $\lambda_*$) & 0.403 [1.000] & 2.315 [0.940] \\ 
  \pkg{IndTestPP} inversion & 0.483 [1.000] & 2.587 [0.684] \\ 
   \hline
\end{tabular}
}
...
We provided functions with the arguments they need to run fastest. For example, functions that use the inversion or order statistics algorithm execute faster when the inverse function $\Lambda^{-1}(z)$ is provided, rather than numerically calculated, as shown in both Figures for the \pkg{nhppp} package. (Functions in other packages do not have this option). The fastest functions are \pkg{nhppp} functions that rely on the inversion or order statistics algorithms given $\Lambda^{-1}(z)$.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{get_piecewise_linear_majorizer}\hlstd{(}\hlkwc{fun} \hlstd{= abs,} \hlkwc{breaks} \hlstd{=} \hlopt{-}\hlnum{5}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{is_monotone} \hlstd{=} \hlnum{FALSE}\hlstd{,}
