...
t <- as.integer(
  seq(min(count$Inversion, count$Thinning),
    max(count$Inversion, count$Thinning),
    length.out = nsim
  )
)
d <- dpois(t, mu)
...
bias_count <- function(x) {
  ...
  return(c(
    mu = mean_obs, BiasN = bN, RBiasN = bNr,
...
comptimes2 <- bench::mark(
  ...
  "(6a) reda: thinning (one)" = reda::simEvent(
    rho = l,
    origin = tp[1], endTime = tp[2],
    recurrent = F,
    method = "thinning"
  ),
  "(6b) reda: inversion (one)" = reda::simEvent(
    rho = l,
    origin = tp[1], endTime = tp[2],
    recurrent = F,
    method = "inversion"
  ),
  "(6c) reda: thinning (all)" = reda::simEvent(
    rho = l,
    origin = tp[1],
    endTime = tp[2],
    recurrent = T,
  ...
  "(6d) reda: inversion (all)" = reda::simEvent(
    rho = l,
    origin = tp[1],
    endTime = tp[2],
    recurrent = T,
