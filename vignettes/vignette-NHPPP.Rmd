...
```{r}
nhppp::sim_nhppp_t_inv(Lambda = L, range_t = tp, rng_stream = S, only1 = FALSE)
```
...
nsim <- 10^4
nhppp_traj <- list(
  inversion = parallel::mclapply(1:nsim, 
                   function(x) nhppp::sim_nhppp_t_inv(Lambda = L, 
                                          range_t = tp,
                                          rng_stream = S, 
                                          only1 = FALSE)
                   ),
  thinning = parallel::mclapply(1:nsim, 
                   function(x) nhppp::sim_nhppp_t_thinning(
                     lambda = l,
                     lambda_maj = l_sup,
                     range_t = tp,
                     rng_stream = S,
...
nhppp_count <- lapply(nhppp_traj, function(x) sapply(x, length))
...
# Results
metrics <- sapply(nhppp_count, bias_count, digits = 3)
metrics
# Latex table
...
# The length of times n=1875 returns 171 events on average (calibrated)
t_seq_1 <- runif(1875, tp[1], tp[2])
...
poisson_thinning = poisson::nhpp.event.times(rate = 27,  # calibrated
...
# We use parallel package functions to speed up replications
cl <- parallel::makeCluster(parallel::detectCores()-1)
parallel::clusterExport(cl, c("packs5", "l", "L", "tp", "r", "l_sup"))
packs5_traj <- parallel::parSapply(cl, X = 1:10^3, FUN = function(i) {i = packs5()})
packs5_traj <- t(packs5_traj)
```
...
packs5_count <-  apply(t(packs5_traj), 2, function(x) sapply(x, length))
metrics2 <- apply(t(packs5_count), 2, bias_count, digits = 3)
metrics2[,1:4]
metrics2[,6:9]
```
...
nhppp_times <- lapply(nhppp_traj, unlist)   
p2 <- do.call(
...
packs5_times <- apply(packs5_traj, 2, unlist)
Name <- names(packs5_times)
p2_1 <- do.call(
  gridExtra::grid.arrange,
  c(lapply(seq_along(packs5_times), bias_times, traj = packs5_times), ncol = 2)
)
