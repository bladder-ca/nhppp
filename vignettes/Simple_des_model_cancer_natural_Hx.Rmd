...
Setting the option `generate_at_least_K = 1` achieves this.

2. We only need to sample the earliest event from this NHPPP. So we set the `report_first K = 1`
option to `TRUE`. 
...
A bit of calculus can yield $N_k(t) = \frac{1}{\beta_k} (e^{\alpha_k + \beta_k t} - e^{\alpha_k})$, which we can implement in vectorized form. The person-specific parameters
$\alpha_k, \beta_k$ arrive through the args container's `row_args` element (a
`data.table` with one row per sampled person), which is passed as the function's
second argument:
...
Then, we can sample the times to cancer death. The same `Lambda_args` container
is delivered to both `Nu` and `Nu_inv`.
...
pop[
  ...
  age_dead_from_cancer_causes := nhppp::vdraw_cumulative_intensity(
    Lambda = Nu,
    Lambda_inv = Nu_inv,
    Lambda_args = list(row_args = pop[!is.na(age_cancer_emergence), ]),
